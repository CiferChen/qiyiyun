...
/**
	 ...
	 * @see com.zhidian.server.model.User
...
public class UserDAO extends BaseHibernateDAO  {
	...
	public static final String USER_ISDELETE = "userIsdelete";
    ...
    public User findById( java.lang.String id) {
        ...
        try {
            User instance = (User) getSession()
                    .get("com.zhidian.server.model.User", id);
            return instance;
        } catch (RuntimeException re) {
    ...
    public List findByExample(User instance) {
        ...
        try {
            List results = getSession()
                    .createCriteria("com.zhidian.server.model.User")
	...
	public List findByUserIsdelete(Object userIsdelete
	) {
		return findByProperty(USER_ISDELETE, userIsdelete
		);
	}
	...
	public List loginWithUserName(String userName,String password){
		try {
	...
	public List loginWithUserEmail(String userEmail,String password){
		try {
			String queryString = "from User as model where model."
					+ userEmail + "= ? and userPassword= ?";
			Query queryObject = getSession().createQuery(queryString);
			queryObject.setParameter(0, userEmail);
			queryObject.setParameter(1, password);
			return queryObject.list();
		} catch (RuntimeException re) {
			log.error("find by property name failed", re);
	...
	public List loginWithUserPhone(String userPhone,String password){
		try {
			String queryString = "from User as model where model."
					+ userPhone + "= ? and userPassword= ?";
			Query queryObject = getSession().createQuery(queryString);
			queryObject.setParameter(0, userPhone);
			queryObject.setParameter(1, password);
			return queryObject.list();
		} catch (RuntimeException re) {
			log.error("find by property name failed", re);
